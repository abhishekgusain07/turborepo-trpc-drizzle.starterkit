  ...
  "inline-flex items-center select-none justify-center light:font-medium rounded-sm w-fit whitespace-nowrap shrink-0 [&>svg]:size-3 gap-1 [&>svg]:pointer-events-none aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive transition-[color,box-shadow] overflow-hidden",
  {
    variants: {
      variant: {
        ...
        secondary:
          "dark:bg-secondary/80 dark:text-secondary-foreground text-white bg-secondary",
        destructive:
          "dark:bg-red-500/10 dark:text-red-500 text-white bg-red-500",
        outline: "text-foreground border",
        green:
          "dark:bg-green-500/10 text-white dark:text-green-500 bg-green-500",
        orange:
          "dark:bg-orange-500/10 text-white dark:text-orange-500 bg-orange-500",
        purple:
          "dark:bg-purple-500/10 text-white dark:text-purple-500 bg-purple-500",
        ...
        yellow:
          "dark:bg-yellow-500/10 dark:text-yellow-500 bg-yellow-500 text-white",
...
function Badge({
  ...
  VariantProps<typeof badgeVariants> & { asChild?: boolean; icon?: boolean }) {
  const Comp = asChild ? Slot : "span";

  return (
    <Comp
      data-slot="badge"
      className={cn(badgeVariants({ variant, size, icon }), className)}
      {...props}
    />
  );
}
